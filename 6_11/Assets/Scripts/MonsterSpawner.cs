...
public class MonsterSpawner : MonoBehaviour
{
    ...
    public GameObject baseObject; // 몬스터 생성 제한 영역으로 사용할 base 오브젝트
    ...
    bool IsPositionValid(Vector3 position)
    {
        ...
        if (baseObject != null)
        {
            ...
            Debug.Log(baseCollider);
            if (baseCollider != null)
            {
                Vector3 baseCenter = baseCollider.bounds.center;
                Vector3 baseSize = baseCollider.bounds.size;
                float baseRadius = Mathf.Max(baseSize.x, baseSize.z) / 2;

                if (Vector3.Distance(position, baseCenter) < baseRadius)
                {
                    return false;
                }
            }
